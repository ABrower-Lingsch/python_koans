...
class AboutStringManipulation(Koan):
    ...
    def test_use_format_to_interpolate_variables(self):
        ...
        #  format asigns a variable to each index
    ...
    def test_any_python_expression_may_be_interpolated(self):
        ...
        # math has many usful mathmatical functions available including sqrt, used to find the square root of the given number
        # in this case we are assigning it to have 4 decimal places
        decimal_places = 4
    ...
    def test_strings_can_be_split_with_different_patterns(self):
        ...
        pattern = re.compile(',|;')
        # ^pattern is telling the split() method how to divide the string up
    ...
    def test_strings_can_be_joined(self):
        ...
        #  ' '.join() starts with a string with only a space. is joins each item in the list into one string each separated by a space
        self.assertEqual("Now is the time", ' '.join(words))

    def test_strings_can_change_case(self):
        self.assertEqual('Guido', 'guido'.capitalize())
        # .capitalize() capitalizes the first char of a string
        self.assertEqual('GUIDO', 'guido'.upper())
        # .upper() makes full string uppercase
        self.assertEqual('timbot', 'TimBot'.lower())
        # .lower() makes full string lowercase
        self.assertEqual('Guido Van Rossum', 'guido van rossum'.title())
        # .title() makes the first char of each word(separated by space) uppercase
        self.assertEqual('tOtAlLy AwEsOmE', 'ToTaLlY aWeSoMe'.swapcase())
        # .swapcase() inverts the case of each char
